...
const _getFrontNotes = async (_cookies: ReadonlyRequestCookies) => {
  try {
    ...
    await reportBackendError({
      path: "/lib/data/notes",
      errorType: "getPublicNotes_error",
      error,
      meta: {},
    });
  }
};
export const getFrontNotes = async () => {
  const _cookies = await cookies();
  const _unstable_cache = unstable_cache(
    async () => await _getFrontNotes(_cookies),
